...
아직 앱에 OBD 연동 코드가 없습니다. 이 문서는 향후 구현 가이드입니다.
...
```
lib/obd/
  ble-manager.ts      ← BleManager 싱글턴, 권한 요청
  elm327.ts           ← AT 명령 직렬화/파싱, 응답 타임아웃
  obd-pids.ts         ← PID 목록, 단위 변환 함수
  obd-service.ts      ← 스캔→연결→초기화→PID 루프 상위 API
```
