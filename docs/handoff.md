...
- `점검`: 앱/SDK 버전, Supabase 연결/카운트/GPS/최근 GPS 경과/중복 진행 운행/진행 운행 표시 수/실기기 확인 안내, 사용자 역할 표시, 역할 변경, PIN 변경(수송부 간부 전용)
- `위치` (`/map`, 수송부 간부 전용): Leaflet.js + OpenStreetMap 지도로 운행 중인 차량의 현재 위치 표시. Supabase Realtime(gps_points INSERT, trips 변경) + 60초 폴백 폴링. 운행 중 차량 없으면 empty state 표시.
