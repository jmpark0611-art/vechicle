...
- `기록`: 최근 30건 기준 목록, 상태/기간/차량 필터, 검색, 필터 결과 요약 통계, 웹 CSV 내보내기, GPS 요약, GPS 누락 경고, 장시간 미종료 경고
- `차량`: 차량 등록/수정/삭제, 차량번호 중복 선검사, 삭제 전 DB 운행 기록 재확인, 검색/상태 필터, 요약 통계, 차량별 운행 상태, 중복 미종료 운행 경고
- `점검`: 앱/SDK 버전, Supabase 연결/카운트/GPS/최근 GPS 경과/중복 진행 운행/실기기 확인 안내
...
- 기록 화면은 성능을 위해 최근 30건을 조회한다. CSV 내보내기는 현재 웹 브라우저에서 동작하며, Expo Go 네이티브 파일 저장은 별도 모듈이 필요하다.
